...
impl EventList {
    ...
    pub fn iter(&self) -> EventListIter<'_> {
        EventListIter {
            count: self.len(),
            packet_ptr: std::ptr::addr_of!(self.0.packet).cast::<MIDIEventPacket>(),
    ...
    pub(crate) unsafe fn as_ptr(&self) -> *const MIDIEventList {
        (self as *const EventList).cast::<MIDIEventList>()
    }
}
...
impl<'a> Iterator for EventListIter<'a> {
    ...
    fn next(&mut self) -> Option<&'a EventPacket> {
        if self.count > 0 {
            let packet = unsafe { &*self.packet_ptr.cast::<EventPacket>() };
            self.count -= 1;
...
impl EventBuffer {
    ...
    pub fn with_capacity(capacity: usize, protocol: Protocol) -> Self {
        ...
        let current_packet_offset = unsafe {
            current_packet_ptr
                .cast::<u8>()
                .offset_from(event_list_ptr.cast::<u8>()) as usize
        };
        Self {
            storage,
    ...
    pub fn push(&mut self, timestamp: Timestamp, data: &[u32]) -> &mut Self {
        ...
        let current_packet_ptr = unsafe {
            self.storage
                .as_mut_ptr::<u8>()
                .add(self.current_packet_offset)
        ...
        self.current_packet_offset = unsafe {
            current_packet_ptr
                .cast::<u8>()
                .offset_from(packet_list_ptr.cast::<u8>()) as usize
        };
    ...
    fn aligned_bytes_len(&self) -> usize {
        if self.as_ref().is_empty() {
            ...
            let current_packet_data_ptr = current_packet.words.as_ptr().cast::<u8>();
...
impl Storage {
        ...
        unsafe {
            match *self {
                Storage::Inline(ref inline) => slice::from_raw_parts(
                    inline.as_ptr().cast::<T>(),
                    inline.len() * size_of::<u32>() / size_of::<T>(),
                ),
                Storage::External(ref vec) => {
                    slice::from_raw_parts(vec.as_ptr().cast::<T>(), vec.len() * 4 / size_of::<T>())
                }
            }
        }
    }
    ...
    pub(crate) unsafe fn as_ptr<T>(&self) -> *const T {
        match *self {
            Storage::Inline(ref inline) => inline.as_ptr().cast::<T>(),
            Storage::External(ref vec) => vec.as_ptr().cast::<T>(),
        }
    }
    ...
    pub(crate) unsafe fn as_mut_ptr<T>(&mut self) -> *mut T {
        match *self {
            Storage::Inline(ref mut inline) => inline.as_mut_ptr().cast::<T>(),
            Storage::External(ref mut vec) => vec.as_mut_ptr().cast::<T>(),
        }
    }
}
...
mod tests {
    ...
    fn event_list_accessors() {
        ...
        let event_list_ptr = buffer.as_mut_ptr().cast::<MIDIEventList>();
