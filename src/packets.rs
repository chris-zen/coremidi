...
impl PacketList {
    ...
    pub(crate) unsafe fn as_ptr(&self) -> *mut MIDIPacketList {
        (self as *const PacketList as *mut PacketList).cast::<MIDIPacketList>()
    }
}
...
impl PacketList {
    ...
    pub fn iter(&self) -> PacketListIterator<'_> {
        PacketListIterator {
            count: self.len(),
            packet_ptr: std::ptr::addr_of!(self.0.packet).cast::<MIDIPacket>(),
...
impl<'a> Iterator for PacketListIterator<'a> {
    ...
    fn next(&mut self) -> Option<&'a Packet> {
        if self.count > 0 {
            let packet = unsafe { &*self.packet_ptr.cast::<Packet>() };
            self.count -= 1;
